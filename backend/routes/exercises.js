...
router.route('/add').post((req, res) => {
    ...
    newExercise.save().then(() => res.json('Exercise add!')).catch(err => res.status(400).json('Error:'+err));
});
...
router.route('/:id').delete((req, res) => {
    Exercise.findByIdAndDelete(req.params.id).then(() => res.json('Deleted!')).catch(err => res.status(400).json('Error:'+err));
});
